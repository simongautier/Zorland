...
exports.parseFilters = json => {
  ...
  if (!wrapper) {
    throw new Error('No sectionListRenderer or richGridRenderer found.');
  }

  const subMenu = wrapper.subMenu || wrapper.submenu;

  if (!subMenu || !subMenu.searchSubMenuRenderer) {
    throw new Error('No searchSubMenuRenderer found in subMenu.');
  }

  const searchSubMenuRenderer = subMenu.searchSubMenuRenderer;

  // Vérifiez si 'groups' existe avant de l'utiliser
  if (!searchSubMenuRenderer.groups) {
    console.log('searchSubMenuRenderer:', searchSubMenuRenderer);
    return new Map(); // Retourner une Map vide si 'groups' n'existe pas
  }

  const filterWrapper = searchSubMenuRenderer.groups;

  if (!Array.isArray(filterWrapper)) {
    throw new Error('filterWrapper is not iterable.');
  }
  ...
  for (const filterGroup of filterWrapper) {
    ...
    if (!filterGroup.searchFilterGroupRenderer || !filterGroup.searchFilterGroupRenderer.filters) {
      continue; // Skip this group if it doesn't have the expected structure
    }
    ...
    for (const filter of filterGroup.searchFilterGroupRenderer.filters) {
      ...
      if (!isSet) {
        targetURL = filter.searchFilterRenderer.navigationEndpoint.commandMetadata.webCommandMetadata.url;
      }
      ...
      if (isSet) {
        singleFilterGroup.active = parsedFilter;
      }
